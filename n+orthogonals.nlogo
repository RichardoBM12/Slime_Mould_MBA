...
patches-own[
  ...
  path-chemial         ;;simulates food communication method - by paths
]
...
to explore-environment ;; turtle procedure
  set neighbourhood other nuclei in-radius 5
  ifelse count neighbourhood < 3 [
    set scout true
    if pcolor != yellow
...
to move ;; turtle procedure
  ifelse scout = true
  [
    if patch-ahead 2 = magenta [ rt random 120 ]                                  ;; Avoid exploring the same paths -> (ortogonal repulsion)
    set neighbourhood other nuclei in-radius 2.5
    if any? neighbourhood [
      find-nearest-neighbor
      if distance nearest-neighbor < avoidance-distance [ collide ]               ;; Avoid nuclei neighbors
    ]
    fd 1
  ] [ fd 1 ]
end
