...
turtles-own[
  ...
  neighbourhood        ;;defines the agent neighbors within a radius
  nearest-neighbor
]
...
patches-own[
  ...
  cohesion-chemical    ;;simulates food communication method - by nuclei
]
...
to go
  ...
  diffuse cohesion-chemical 1                                               ;; diffuse chemical to neighboring patches

  ask patches with [ oat = false ]
  [ set cohesion-chemical cohesion-chemical * 0.9                           ;; evaporate chemical
    set pcolor scale-color yellow cohesion-chemical 0.1 3 ]
  tick
end
...
;; Process of exploration and creation of paths
to explore-environment ;; turtle procedure
  ifelse [cohesion-chemical] of patch-here <= 0.3
  [
    if scout = false [ set scout true ]          ;; only scouts create paths
    if pcolor != yellow
...
to move ;; turtle procedure
  ifelse scout = true
  [
    ;; Avoid exploring the same path
    if pcolor = yellow [ rt 180 ]
    set neighbourhood other nuclei in-radius 2.5
    ;; Avoid nuclei neighbors
    if any? neighbourhood [
      find-nearest-neighbor
      if distance nearest-neighbor < avoidance-distance [ avoid ]
    ]
    fd 1
  ] [ fd 1 ]
end
...
to avoid ;; turtle procedure
  let collision-heading [heading] of nearest-neighbor
  set heading (collision-heading - 180)
end
